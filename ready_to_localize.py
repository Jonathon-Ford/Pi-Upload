...
from pypozyx import (POZYX_POS_ALG_UWB_ONLY, POZYX_3D, Coordinates, POZYX_SUCCESS, PozyxConstants, version,
                     DeviceCoordinates, PozyxSerial, get_first_pozyx_serial_port, SingleRegister, DeviceList, PozyxRegisters, EulerAngles, Acceleration)
...
class ReadyToLocalize(object):
 ...
 -1675                 (operation, self.pozyx.getErrorMessage(error_code)))
            if self.osc_udp_client is not None:
...
if __name__ == "__main__":
    ...
    remote_id = 0x673c                 # remote device network ID
